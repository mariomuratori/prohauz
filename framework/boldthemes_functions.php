...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'prohauz' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'prohauz' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php echo esc_html__( 'Pingback:', 'prohauz' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'prohauz' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'prohauz' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'prohauz' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php echo esc_html__( 'out of 5', 'prohauz' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'prohauz' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'prohauz' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'prohauz' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'prohauz' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'prohauz' ) . '\';';
		return $data;
	}
}
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'prohauz' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'prohauz' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				//if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
					...
					$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'prohauz' );
					$subtitle = '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'prohauz' ) . ' ' . $sku . '</span>'; 
				...
				if ( ! is_null( boldthemes_get_option( 'pf_slug' ) ) && boldthemes_get_option( 'pf_slug' ) != '' ) {
					...
					$output .= $item_prefix . esc_html__( 'Portfolio', 'prohauz' ) . $item_suffix;
				} else {
					$output .= $item_prefix . esc_html__( 'Portfolio', 'prohauz' ) . $item_suffix;
				}
				...
				$output .= $item_prefix . esc_html__( 'Category', 'prohauz' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'prohauz' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'prohauz' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'prohauz' ) . $item_suffix;
